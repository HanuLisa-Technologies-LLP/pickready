"""The six-month window: what a candidate is told about an earlier assessment.

NOTHING TRAVELS BETWEEN JOBS, AGAIN (2026-09-25). The 2026-09-22 ruling
(change request 23) let a candidate's PORTABLE evidence stand in for asking a
criterion on a later job, and the window below decided whether a portable fact
was fresh enough to do so. Phase 3 of the Vivekium release SUPERSEDES that:
every item of every assessment is asked (Appendix B), the portable evidence
module is deleted, and so is the consent item that authorised the reuse
(`consent_catalog.RETIRED_KEYS`). What survives is the part of this module
that never depended on reuse: the classification that lets a candidate be
told why they are answering questions again.

WHY NO REPORT SECTION TRAVELS (2026-07-30, unchanged)
------------------------------------------------------
Each job's skills are its own, so copying a report across would state a grade
against criteria the candidate was never assessed on. `PORTABLE_CATEGORIES`
stays an empty frozenset and `copy_report` copies nothing;
`app/scripts/eval_report.py` gates on the emptiness.
...
Phase 5 (WP5-G) deletes this module once its last caller, the invitation
resolve in `api/assessment_conversation.py`, is gone (Phase 3 WP3 removes it).
"""
...
class RetakeDecision:
    ...
    def requires_new_assessment(self) -> bool:
        # Always true: every application runs its own assessment. The property
        # is kept because callers read it rather than the string.
        return True
    ...
    def message(self) -> str | None:
        """The sentence shown to the candidate. None for a first assessment,
        which needs no preamble. Months are WORDS about time, never a count
        of anything assessed."""
...
async def decide(
    ...
    *,
    ...
    dimension from it reaches the returned decision.
    """
