...
#: The schemas still allowed a raw number: NONE. D3 took `DashboardRowOut`'s
#: licence, and the audited calibration view (`CalibrationInternalsOut`,
#: `CalibrationDimensionOut`) was deleted with its route in the Vivekium
#: release (PLAN-p7 WP-B6). Kept as a named empty set so the walk below states
#: its exception list rather than having none to state.
NUMERIC_SCHEMAS: dict[str, str] = {
}
...
def test_no_schema_is_licensed_to_carry_a_number():
    """The allow-list is empty and stays empty. A stale entry is a hole nobody
    notices reopening, and after D3 and WP-B6 no entry is a live one."""
