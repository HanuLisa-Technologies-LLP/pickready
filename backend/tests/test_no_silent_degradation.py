...
#: stays only until its deletion (Phase 7 WP-B5) merges.
...
#: the Phase 7 WP-B7 fixes. Same ratchet rule as above: it may shrink, it may
#: not grow. Each entry names its owner.
#:
#:   api/billing.py                 the Razorpay webhook answers ANY flush
#:                                  error as a duplicate. Phase 7 WP-B6
#:                                  narrows it to the unique violation.
...
#:   services/memory/, reasoning/,  unreachable subsystems and the MSG91
#:   services/sms_service.py        sender. Deleted by Phase 7 Wave B.
...
#:   scripts/validate_ppi.py,       operator CLI checks whose handler IS the
#:   scripts/validate_stack.py      verdict (`refused = True`, WARN in the
#:                                  caller); they never run on a request.
LEGACY_SILENT_HANDLER_FILES: frozenset[str] = frozenset(
    {
        ...
        "services/functional_assessment.py",
        "services/interviewer.py",
        "services/memory/procedural.py",
        "services/projects/invisible_text.py",
        "services/projects/parsers.py",
        "services/reasoning/runner.py",
