...
// Nav-gated by capability `configure_approval_levels`. Approver candidates
// come from the staff list (contract rev 2) filtered to Hiring Managers.
...
export default function ApprovalLevelsPage() {
  ...
  const [managers, setManagers] = React.useState<StaffMember[]>([]);
  const [saving, setSaving] = React.useState(false);

  React.useEffect(() => {
    apiGet<StaffMember[] | { staff: StaffMember[] }>("/companies/me/staff")
      .then((res) => {
        const all = Array.isArray(res) ? res : res.staff ?? [];
        setManagers(
          all.filter(
            (m) =>
              m.role === "hiring_manager" &&
              m.status?.toLowerCase() !== "disabled"
          )
        );
      })
      .catch(() => {});
