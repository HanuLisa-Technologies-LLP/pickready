...
2. **No passwords, anywhere.** All auth is OTP. If you find yourself adding a password field or a "forgot password" flow, stop — that's the wrong pattern for this product.
...
5. **Client-domain email only goes through the tenant's verified Resend sending domain.** Never wire up Gmail/Outlook OAuth for outbound mail — this was explicitly ruled out by the client.
