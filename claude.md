...
5. **All outbound email goes through SMTP from the backend (as of 2026-07-24, replaces the Mailtrap HTTP API).** A provider-agnostic SMTP sender configured entirely via env (`SMTP_HOST/PORT/USER/PASSWORD/FROM`, TLS flags) — point it at Mailtrap's SMTP, Gmail SMTP (app password), or any provider. No Gmail/Outlook **OAuth** anywhere (SMTP app-password auth is fine). Sending is still a Celery task with the permanent-vs-transient failure taxonomy.
