...
resumes_data = [
    ...
    # 18 - Machine Learning Engineer
    {
        ...
        "certifications": [
            "Machine Learning Specialization by Standford Online (Coursera)"
        ],
        "achievements": [
...
def format_resume_text(data, email, phone):
    num = data["num"]
    name = data["name"]
    role = data["role"]
    ...
    github = f"github.com/{name.lower().replace(' ', '')}-dev"
    
    lines = []
    lines.append(f"Resume {num}")
    lines.append("")
...
for idx, res in enumerate(resumes_data):
    ...
    content = format_resume_text(res, email, phone)
    
    filename_txt = f"Resume_{res['num']}_{res['name'].replace(' ', '_')}.txt"
    filepath_txt = os.path.join(output_dir, filename_txt)
    with open(filepath_txt, "w", encoding="utf-8") as f:
        f.write(content)
        
    filename_md = f"Resume_{res['num']}_{res['name'].replace(' ', '_')}.md"
    filepath_md = os.path.join(output_dir, filename_md)
    with open(filepath_md, "w", encoding="utf-8") as f:
        f.write(content)

print("Successfully created 30 resume documents in", output_dir)
